...
import { mockRemoteId } from "$server/connectors/mock";
 ...
 * is the queue driver's own BACKOFF_MS, and the remote IDs are generated by the
 * same mockRemoteId the mock connectors call. A marketing page that drifts from
...
export const load: PageServerLoad = async () => {
  ...
  const channels = Object.values(connectors).map((c) => {
    ...
    return {
      ...
      regions: m.regions,
      remoteId: mockRemoteId(m.idPrefix, sku),
  ...
  return {
    ...
    channels,
    byGroup,
    groups: CONNECTOR_GROUPS.map((g) => g.label),
